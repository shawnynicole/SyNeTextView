...
open class SyNeTextView: CenterableTextView {
    ...
    private lazy var placeholderView: CenterableTextView = {
       
        let view = CenterableTextView()
    ...
    lazy var doneButton: UIButton = {
        ...
        button.titleLabel?.font = button.titleLabel?.font._bold()
    ...
    open override var shouldCenterVertically: Bool {
        get { return super.shouldCenterVertically }
        set {
            super.shouldCenterVertically = newValue
            placeholderView.shouldCenterVertically = newValue
        }
    }
    ...
    open override func layoutSubviews() {
        ...
        // Keeps placeholder from being pushed down due to centering of self
        placeholderView.frame.origin = contentOffset
        placeholderView.frame.size = bounds.size
...
//        {
//            if let range = tokenizer.rangeEnclosingPosition(textPosition, with: .word, inDirection: 1)
//            {
...
//    }
    ...
    // If used as a TextField, do not make it selectable
...
//            }
        ...
        if let collectionView = _traverseSyNeTextViewHierarchy(UICollectionView.self) {
            guard let cell = _traverseSyNeTextViewHierarchy(UICollectionViewCell.self) else { return }
        ...
        else if let tableView = _traverseSyNeTextViewHierarchy(UITableView.self) {
            
            guard let cell = _traverseSyNeTextViewHierarchy(UITableViewCell.self) else { return }
